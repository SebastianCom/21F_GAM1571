...
void Game::Init()
{
	...
	for (int i = 0; i < static_cast<int>(m_pPlayer->RandomFloat(5.0f, 15.0f)); i++)
	{
		...
		if (i > 0)
		{
			for (int j = 0; j < i; j++)
			{
				if (m_vecEnemies.at(i)->CheckCollision(m_vecEnemies.at(j), m_vecEnemies.at(i)->GetX(), m_vecEnemies.at(i)->GetY()) == true)
				{
...
void Game::Update(float deltaTime)
{
	...
	ImGui::Text("Player X: %d", m_pPlayer->GetX());
	ImGui::Text("Player Y: %d", m_pPlayer->GetY());
	...
	for (int i = 0; i < m_vecEnemies.size(); i++)
	{
		if (m_vecEnemies.at(i)->GetActive() == true)
		{
			...
			{
				//Todo make sure the player respwans not touching anything
